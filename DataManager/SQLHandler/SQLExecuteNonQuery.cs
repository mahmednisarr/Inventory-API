...
namespace INV.DataManager
{
    public partial class SQLExecuteNonQuery : SQLHandler
    {
        ...
        public SQLExecuteNonQuery(IConfiguration config, IHttpContextAccessor httpContextAccessor)
        {
            SQLHandler sQLHandler = new SQLHandler(config, httpContextAccessor);
            _config = config;
            dbName = _httpContextAccessor.HttpContext.Items["User"]!=null ? _httpContextAccessor.HttpContext.Items["User"].ToString():"";
