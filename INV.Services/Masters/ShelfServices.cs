...
namespace INV.Services.Masters
{
    public class ShelfServices : IShelfServices
    {
        ...
        public ShelfServices(IHttpContextAccessor httpContext, IConfiguration _config)
        {
        ...
        {
            DataSet _dataSet = await _dataWare.GetDataSet("I_Shelf_get", _list);
            ...
            if (_dataSet != null && _dataSet.Tables.Count > 0)
            {
                ...
                _response.ResponsePacket = _dataSet.Tables[0].DataTableToList<ShelfDto>();
        ...
        public async Task<TResponse> DeleteShelf(int ID, int usrID)
        {
            try
            {
                _list.Add(new ViewParam() { Name = "userID", Value = usrID });
                _list.Add(new ViewParam() { Name = "id", Value = ID });
                DataSet _dataSet = await _dataWare.GetDataSet("I_Shelf_Delete", _list);
        ...
        public async Task<TResponse> SaveOrUpdateShelf(ShelfDto reqResponseDto, int usrID)
        {
            try
            {
                ...
                _list.Add(new ViewParam() { Name = "rackID", Value = reqResponseDto.RackID });
                _list.Add(new ViewParam() { Name = "usrId", Value = usrID });
                _dataSet = await _dataWare.GetDataSet("I_Shelf_Save", _list);
