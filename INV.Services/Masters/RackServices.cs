...
{
    public class RackService : IRackServices
    {
        DataWare _dataWare;
        DataManager _DM;
        ...
        public RackService(IHttpContextAccessor httpContext, IConfiguration _config)
        {
        ...
        {
            DataSet _dataSet = await _dataWare.GetDataSet("I_Rack_get", _list);
            ...
            if (_dataSet != null && _dataSet.Tables.Count > 0)
            {
                ...
                _response.ResponsePacket = _dataSet.Tables[0].DataTableToList<RackDto>();
            }
            else
            {
        ...
        public async Task<TResponse> DeleteRack(int ID, int usrID)
        {
            try
            {
                _list.Add(new ViewParam() { Name = "userID", Value = usrID });
                _list.Add(new ViewParam() { Name = "id", Value = ID });
                DataSet _dataSet = await _dataWare.GetDataSet("I_Rack_Delete", _list);
        ...
        public async Task<TResponse> SaveOrUpdateRack(RackDto reqResponseDto, int usrID)
        {
            try
            {
                ...
                _list.Add(new ViewParam() { Name = "deptID", Value = reqResponseDto.DeptID });
                _list.Add(new ViewParam() { Name = "usrId", Value = usrID });
                _dataSet = await _dataWare.GetDataSet("I_Rack_Save", _list);
